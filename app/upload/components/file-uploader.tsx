  ...
  const handleFileChange = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      for (const file of Array.from(files)) {
        ...
        try {
          const result = await addInvoice(file);
          successCount++;

          // Verificar si hay advertencias en la factura recién cargada
          if (result.validacion) {
            ...
            if (result.validacion.errores.length > 0) {
              warningCount++;
              warnings.push(...result.validacion.errores);
            }
          }
        } catch (err) {
          errorCount++;
  ...
  return (
    <div className="mb-6">
      <div
        ...
        onDrop={handleDrop}
        className="border-2 border-dashed border-gray-300 rounded-lg p-12 text-center hover:border-primary transition-colors bg-white"
      >
        <input
          type="file"
